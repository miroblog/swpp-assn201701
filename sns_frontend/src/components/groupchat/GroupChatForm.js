...
import ExitGroupchatButton from './ExitGroupchatButton';
...
class GroupChatForm extends React.Component {
  ...
  chatUsers() {
    ...
    for (let i=0; i<this.props.users.length; i++) {
      if(this.props.user != this.props.users[i]) {
...
//Chat Later Button Added to Initialize state
  render() {
    const photo_input = this.getPhotoInput();
	  var placeholder = "Write a message to your Group!"
    return (
      <div>
        <div>
          <div className="jumbotron">
					  <h3 id='chatUsers'>Chatting with {this.chatUsers()}...{this.props.valid}</h3>
		    		<AddUserButton />
            <div className="pull-right" style={{'text-align':'right'}}>
              <button id='chatlater' className="btn btn-warning btn-sm" onClick={this.chatlater}>Chat Later!</button>
              <ExitGroupchatButton/>
            </div>
          </div>
          {this.renderMessages()}
...
function mapStateToProps(state) {
    return {
      ...
      users: state.groupchat.users,
	    group_id: state.groupchat.group_id,
    };
}
...
export default connect(mapStateToProps, mapDispatchToProps)(GroupChatForm);
